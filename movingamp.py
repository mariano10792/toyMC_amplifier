...
for a in A:
    for x0 in X:
        for c in C:
            ...
            space=np.zeros([w+7,h+1]) #this matrix "moves".
            MU=np.zeros([w+7,h+1])
            for x in range(w+7):
                for y in range(h+1):
                    MU[x][y]=a/(float(x+x0+.1)**2+float(y-1+.01)**2)+c
                    # MU[x][y]=1
            ...
            for contadory in range(1,h+1): # number of times I move rows
                for ps in range(PSLEN,-1,-1): #PreScan. Actually moves the SR until the first column of the contadory-1 row reaches the amplifier
                    for aalenx in range(0,AALENX,1):
                        for aaleny in range(0,AALENY-contadory,1):
                        # SPACE[0,0]+=MU[ps,0]
                        # SPACE[1,0]+=MU[ps+1,0]
                        # ...
